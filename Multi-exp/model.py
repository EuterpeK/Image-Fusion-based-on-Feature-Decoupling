...
class CGAN(object):
  ...
  def build_model(self):
    ...
    with tf.name_scope('g_loss'):
        # self.vi_loss = 1.5*tf.reduce_mean(tf.square(self.vi_fusion - self.labels_vi)) + 1.8*(1- tf.reduce_mean(tf.image.ssim(self.labels_vi, self.vi_fusion, 1.0))) + \
        #                1.8*tf.reduce_mean(1-tf.image.ssim(self.labels_vi,self.vi_gradient,1.0)) + 1.5* tf.reduce_mean(tf.square(self.labels_vi - self.vi_res)) 
        ...
        # self.ir_loss = 1.5*tf.reduce_mean(tf.square(self.ir_fusion - self.labels_ir)) + 1.8*tf.reduce_mean(1-tf.image.ssim(self.labels_ir, self.ir_fusion, 1.0)) + \
        #                1.8*tf.reduce_mean(1-tf.image.ssim(self.labels_ir, self.vi_res, 1.0)) + 1.5* tf.reduce_mean(tf.square(self.labels_ir-self.ir_intensity))
        
        # self.g_loss_2= 1 * self.vi_loss + 1*self.ir_loss + \
        #                1.75*tf.reduce_mean(tf.square(self.fusion_image - self.labels_ir))+ 1.5*tf.reduce_mean(tf.square(self.fusion_image - self.labels_vi))+ \
        #                17.2*tf.reduce_mean(1-tf.image.ssim(self.labels_vi, self.fusion_image, 1.0))+ 17*tf.reduce_mean(1-tf.image.ssim(self.labels_ir, self.fusion_image, 1.0))
        ...
        # ir_int = vi_int=1.1 ir_int=1.5 vi_int=1.4
