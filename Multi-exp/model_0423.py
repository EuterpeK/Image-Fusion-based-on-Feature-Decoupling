  ...
  def build_model(self):
    ...
    with tf.name_scope('g_loss'):
        ...
        self.g_loss_2= 1 * self.vi_loss + 1*self.ir_loss + \
                       1.6*tf.reduce_mean(tf.square(self.fusion_image - self.labels_ir))+ 1.8*tf.reduce_mean(tf.square(self.fusion_image - self.labels_vi))+ \
                       22*tf.reduce_mean(1-tf.image.ssim(self.labels_vi, self.fusion_image, 1.0))+ 21*tf.reduce_mean(1-tf.image.ssim(self.labels_ir, self.fusion_image, 1.0))
