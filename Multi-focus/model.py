...
class CGAN(object):
  ...
  def __init__(self, 
               ...
               vi_loss=1.0,
               ir_loss=1.0,
               fusion_vi_gradient=3,
               fusion_vi_intensity=2,
               fusion_ir_gradient=3,
               fusion_ir_intensity=2,
               dir_name='auto'):
  ...
  def build_model(self):
    ...
    with tf.name_scope('g_loss'):
        self.vi_loss = 2*tf.reduce_mean(tf.square(self.vi_fusion - self.labels_vi)) + 3*tf.reduce_mean(1- (tf.image.ssim(self.labels_vi, self.vi_fusion, 1.0))) + \
                       3*tf.reduce_mean(1-tf.image.ssim(self.labels_vi,self.vi_gradient,1.0)) + 2* tf.reduce_mean(tf.square(self.labels_vi - self.vi_res)) 
        self.ir_loss = 2*tf.reduce_mean(tf.square(self.ir_fusion - self.labels_ir)) + 3*tf.reduce_mean(1-tf.image.ssim(self.labels_ir, self.ir_fusion, 1.0)) + \
                       3*tf.reduce_mean(1-tf.image.ssim(self.labels_ir, self.vi_res, 1.0)) + 2* tf.reduce_mean(tf.square(self.labels_ir-self.ir_intensity))
        
        self.g_loss_2= 1 * self.vi_loss + 1*self.ir_loss + \
                       11*tf.reduce_mean(tf.square(self.fusion_image - self.labels_ir))+ 10*tf.reduce_mean(tf.square(self.fusion_image - self.labels_vi))+ \
                       16.2*tf.reduce_mean(1-tf.image.ssim(self.labels_vi, self.fusion_image, 1.0))+ 16.001*tf.reduce_mean(1-tf.image.ssim(self.labels_ir, self.fusion_image, 1.0))
  ...
  def train(self, config):
    if config.is_train:
      input_setup(self.sess, config,"../Multi-focus/Train_ir")
      input_setup(self.sess,config,"../Multi-focus/Train_vi")
    else:
      nx_ir, ny_ir = input_setup(self.sess, config,"../Multi-focus/Test_ir")
      nx_vi,ny_vi=input_setup(self.sess, config,"../Multi-focus/Test_vi")

    if config.is_train:     
      data_dir_ir = os.path.join('./{}'.format(config.checkpoint_dir), "../Multi-focus/Train_ir","train.h5")
      data_dir_vi = os.path.join('./{}'.format(config.checkpoint_dir), "../Multi-focus/Train_vi","train.h5")
    else:
      data_dir_ir = os.path.join('./{}'.format(config.checkpoint_dir),"../Multi-focus/Test_ir", "test.h5")
      data_dir_vi = os.path.join('./{}'.format(config.checkpoint_dir),"../Multi-focus/Test_vi", "test.h5")
  ...
  def ir_decoder(self, ir_concat):
    with tf.variable_scope('ir_decoder'):
      with tf.variable_scope('ir_layer1'):
        weights = tf.get_variable('ir_w1', [3,3,2,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('ir_b1',[16],initializer=tf.constant_initializer(0.0))
        layer1 = tf.nn.conv2d(ir_concat,weights,strides=[1,1,1,1],padding='SAME')+bias
        layer1 = lrelu(layer1)
      with tf.variable_scope('ir_layer2'):
        weights = tf.get_variable('ir_w2', [1,1,16,1], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('ir_b2',[1],initializer=tf.constant_initializer(0.0))
        ir_fusion = tf.nn.conv2d(layer1,weights,strides=[1,1,1,1],padding='SAME')+bias
  ...
  def vi_decoder(self, vi_concat):
    with tf.variable_scope('vi_decoder'):
      with tf.variable_scope('vi_layer1'):
        weights = tf.get_variable('vi_w1', [3,3,2,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('vi_b1',[16],initializer=tf.constant_initializer(0.0))
        layer1 = tf.nn.conv2d(vi_concat,weights,strides=[1,1,1,1],padding='SAME')+bias
        layer1 = lrelu(layer1)
      with tf.variable_scope('vi_layer2'):
        weights = tf.get_variable('vi_w2', [1,1,16,1], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('vi_b2',[1],initializer=tf.constant_initializer(0.0))
        vi_fusion = tf.nn.conv2d(layer1,weights,strides=[1,1,1,1],padding='SAME')+bias
