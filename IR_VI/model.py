...
class CGAN(object):
  ...
  def build_model(self):
    ...
    with tf.name_scope('input'):
        ...
        self.input_intensity =tf.concat([self.ir_intensity,self.ir_intensity,self.vi_res],axis=-1)
        self.input_gradient =tf.concat([self.vi_gradient,self.vi_gradient,self.ir_res],axis=-1)
    ...
    with tf.name_scope('g_loss'):
        ...
        self.vi_loss = 0.3*tf.reduce_mean(tf.square(self.vi_fusion - self.labels_vi)) + 5 * tf.reduce_mean(tf.square(gradient(self.vi_fusion)-gradient(self.labels_vi))) + 5 * tf.reduce_mean(tf.square(gradient(self.labels_vi)-self.vi_gradient)) + 0.3*tf.reduce_mean(tf.square(self.vi_res) - self.labels_vi)
        self.ir_loss = 1*tf.reduce_mean(tf.square(self.ir_fusion - self.labels_ir)) + 4 * tf.reduce_mean(tf.square(gradient(self.ir_fusion)-gradient(self.labels_ir))) + 1 * tf.reduce_mean(tf.square(self.labels_ir-self.ir_intensity)) + 4 * tf.reduce_mean(tf.square(self.labels_ir-self.ir_res))
        self.g_loss_2=self.vi_loss + self.ir_loss + 1*tf.reduce_mean(tf.square(self.fusion_image - self.labels_ir))+0.3*tf.reduce_mean(tf.square(self.fusion_image - self.labels_vi))+5*tf.reduce_mean(tf.square(gradient(self.fusion_image) - self.labels_vi))+4*tf.reduce_mean(tf.square(gradient(self.fusion_image) - gradient(self.labels_ir)))
  ...
  def train(self, config):
    if config.is_train:
      input_setup(self.sess, config,"../../IR_VI/Train_ir")
      input_setup(self.sess,config,"../../IR_VI/Train_vi")
    else:
      nx_ir, ny_ir = input_setup(self.sess, config,"../../IR_VI/Test_ir")
      nx_vi,ny_vi=input_setup(self.sess, config,"../../IR_VI/Test_vi")

    if config.is_train:     
      data_dir_ir = os.path.join('./{}'.format(config.checkpoint_dir), "../../IR_VI/Train_ir","train.h5")
      data_dir_vi = os.path.join('./{}'.format(config.checkpoint_dir), "../../IR_VI/Train_vi","train.h5")
    else:
      data_dir_ir = os.path.join('./{}'.format(config.checkpoint_dir),"../../IR_VI/Test_ir", "test.h5")
      data_dir_vi = os.path.join('./{}'.format(config.checkpoint_dir),"../../IR_VI/Test_vi", "test.h5")
  ...
  def ir_encoder(self, img_ir):
    ...
    with tf.variable_scope('ir_encoder'):
      with tf.variable_scope('ir_layer1'):
        weighs = tf.get_variable('ir_w1',[7,7,1,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('ir_b1',[16],initializer=tf.constant_initializer(0.0))
      ...
      with tf.variable_scope('ir_layer2'):
        weighs = tf.get_variable('ir_w2',[7,7,16,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
      ...
      with tf.variable_scope('ir_layer3'):
        weights = tf.get_variable('ir_w3',[5,5,32,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('ir_b3',[16],initializer=tf.constant_initializer(0.0))
        layer3 =  tf.contrib.layers.batch_norm(tf.nn.conv2d(concat1,weights,strides=[1,1,1,1],padding='SAME')+bias, decay=0.9,  updates_collections=None, epsilon=1e-5, scale=True     )
        layer3 = lrelu(layer3)

      concat2 = tf.concat([layer1, layer2, layer3],axis=-1)

      with tf.variable_scope('ir_intensity'):
        weighs = tf.get_variable('ir_intensity_w',[5,5,48,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('ir_intensity_b',[16],initializer=tf.constant_initializer(0.0))
        intensity =  tf.contrib.layers.batch_norm(tf.nn.conv2d(concat2,weighs,strides=[1,1,1,1],padding='SAME')+bias, decay=0.9,  updates_collections=None, epsilon=1e-5, scale=True     )
        intensity = lrelu(intensity)
      
      with tf.variable_scope('ir_res'):
        weighs = tf.get_variable('ir_res_w',[5,5,48,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('ir_res_b',[16],initializer=tf.constant_initializer(0.0))
        res =  tf.contrib.layers.batch_norm(tf.nn.conv2d(concat2,weighs,strides=[1,1,1,1],padding='SAME')+bias, decay=0.9,  updates_collections=None, epsilon=1e-5, scale=True     )
        res = lrelu(res)
  ...
  def vi_encoder(self, img_vi):
    ...
    with tf.variable_scope('vi_encoder'):
      with tf.variable_scope('vi_layer1'):
        weighs = tf.get_variable('vi_w1',[7,7,1,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('vi_b1',[16],initializer=tf.constant_initializer(0.0))
      ...
      with tf.variable_scope('vi_layer2'):
        weighs = tf.get_variable('vi_w2',[7,7,16,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
      ...
      with tf.variable_scope('vi_layer3'):
        weighs = tf.get_variable('vi_w3',[7,7,32,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
      ...
      with tf.variable_scope('vi_gradient'):
        weighs = tf.get_variable('vi_gradient_w',[7,7,48,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('vi_gradient_b',[16],initializer=tf.constant_initializer(0.0))
        gradient =  tf.contrib.layers.batch_norm(tf.nn.conv2d(concat2,weighs,strides=[1,1,1,1],padding='SAME')+bias, decay=0.9,  updates_collections=None, epsilon=1e-5, scale=True     )
        gradient = lrelu(gradient)
      
      with tf.variable_scope('vi_res'):
        weighs = tf.get_variable('vi_res_w',[5,5,48,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('vi_res_b',[16],initializer=tf.constant_initializer(0.0))
        res =  tf.contrib.layers.batch_norm(tf.nn.conv2d(concat2,weighs,strides=[1,1,1,1],padding='SAME')+bias, decay=0.9,  updates_collections=None, epsilon=1e-5, scale=True     )
        res = lrelu(res)
  ...
  def ir_decoder(self, ir_concat):
    with tf.variable_scope('ir_decoder'):
      with tf.variable_scope('ir_layer1'):
        weights = tf.get_variable('ir_w1', [5,5,32,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('ir_b1',[16],initializer=tf.constant_initializer(0.0))
        ...
        layer1 = lrelu(layer1)
      with tf.variable_scope('ir_layer2'):
        weights = tf.get_variable('ir_w2', [5,5,16,1], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('ir_b2',[1],initializer=tf.constant_initializer(0.0))
  ...
  def vi_decoder(self, vi_concat):
    with tf.variable_scope('vi_decoder'):
      with tf.variable_scope('vi_layer1'):
        weights = tf.get_variable('vi_w1', [7,7,32,16], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('vi_b1',[16],initializer=tf.constant_initializer(0.0))
        ...
        layer1 = lrelu(layer1)
      with tf.variable_scope('vi_layer2'):
        weights = tf.get_variable('vi_w2', [7,7,16,1], initializer=tf.truncated_normal_initializer(stddev=1e-3))
        bias = tf.get_variable('vi_b2',[1],initializer=tf.constant_initializer(0.0))
...
####################  Layer1  ###########################
    with tf.variable_scope('fusion_model'):
        with tf.variable_scope('layer1'):
            weights=tf.get_variable("w1",[5,5,48,16],initializer=tf.truncated_normal_initializer(stddev=1e-3))
        ...
        with tf.variable_scope('layer1_vi'):
            weights=tf.get_variable("w1_vi",[5,5,48,16],initializer=tf.truncated_normal_initializer(stddev=1e-3))
