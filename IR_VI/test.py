...
for idx_num in range(15):
  ...
  while(num_epoch==idx_num):
      model_dir = '0304_1'
      reader = tf.compat.v1.train.NewCheckpointReader('./checkpoint/'+model_dir+'/CGAN.model-1')
      ...
      with tf.compat.v1.Session() as sess:
        ...
        #   data_vi=prepare_data('../input/d/euterpek/medical/Medical/Test_vi')
          data_ir=prepare_data('../../IR_VI/Test_ir')
          data_vi=prepare_data('../../IR_VI/Test_vi')
          for i in range(len(data_ir)):
              ...
              ir_result = sess.run(ir_fusion, feed_dict={images_ir: train_data_ir,images_vi: train_data_vi})
              vi_result = sess.run(vi_fusion, feed_dict={images_ir: train_data_ir,images_vi: train_data_vi})
              ...
              if i<=9:
                  fusion_path = os.path.join(image_path,'F9_0'+str(i)+".bmp")
              else:
                  fusion_path = os.path.join(image_path,'F9_'+str(i)+".bmp")
              ir_path = os.path.join(image_path, 'IR_' + str(i) + ".bmp" )
              vi_path = os.path.join(image_path, 'VI_' + str(i) + ".bmp" )
              end=time.time()
              # print(out.shape)
              imsave(result, fusion_path)
              imsave(ir_result, ir_path)
              imsave(vi_result, vi_path)
